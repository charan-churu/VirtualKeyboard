...
function Keyboard() {
    ...
    const handleKeyClick=(key)=>{
      ...
      else if(key===' '){handleSpaceKey();}
      else if(key==='Caps Lock'){handleCapsLock()}
      else if (key === 'backspace') {handleDeleteKey()} 
      else if (key === 'Shift') { handleShiftKey();} 
      else if (key === 'Tab') {handleTabKey();}
      else {handleRegularKey(key);}
    }
    const handleSpaceKey=()=>{
      const newComponent=inputText+'\u00A0'
      setInputText(newComponent)
    }
